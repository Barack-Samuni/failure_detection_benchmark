...
from configs.load_model_and_config import (
    get_config_data_model_for_eval_train,
...
def evaluate_single_model(
    ...
    if get_failure_detection_filename(output_dir).exists():
        ...
        scores_dict = get_baseline_scores(train_output_baseline, threshold)
    ...
    if "doctor_alpha" in scores_dict.keys():
        ...
        scores_dict["doctor_alpha"] = get_doctor_alpha_scores(train_output_baseline)
    ...
    if getattr(model, "p_dropout", 0) > 0:
        ...
        else:           # we are only intrested in the train set for now
            assert hasattr(
    ...
    else:           # Again, we are only interested in train set
    ...
    if "ConfidNet_scores" in scores_dict.keys():
        ...
        confidnet_scores = get_confidnet_scores(
            deepcopy(model), data_module.train_dataloader(), output_dir
        )
    ...
    # Get DUQ   - Again - only train, but we actually didn't train it so this code won't run
...
def get_all_scores(
    ...
    config, data_module, models, output_dirs = get_config_data_model_for_eval_train(
        config_name
    )
    all_dfs = []
